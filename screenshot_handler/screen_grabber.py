...
class ScreenGrabber:
    def __init__(self, dest_folder, monitor=None):
        ...
        self.monitor = monitor if monitor is None else __parse_monitor(monitor)
        self.sct = mss.mss()
    ...
    def grab_screenshot(self, index):
        output_path = self.dest_folder + __get_screenshot_name(index)
        if self.monitor is None:
            self.__grab_fullscreen(output_path)
        else:
            self.__grab_part_screen(output_path)

def __parse_monitor(monitor):
...
def __get_screenshot_name(index):
